...
export default function LessonDiscussion({
  ...
  const handlePost = async () => {
    ...
    if (c) {
      if (replyTo) {
        setComments((prev) =>
          prev.map((p) => (p.id === replyTo ? { ...p, replies: [...(p.replies ?? []), c] } : p)),
        );
      } else {
  ...
  return (
    <div className="space-y-4">
      <p className="inline-flex items-center gap-2 font-tech text-[10px] uppercase tracking-[0.2em] text-cyan-200">
        <MessageCircle size={12} /> Thảo luận (
        {comments.reduce((s, c) => s + 1 + (c.replies?.length ?? 0), 0)})
      </p>
      ...
      <div className="flex gap-2">
        ...
        {replyTo && (
          <button
            ...
            className="text-xs text-secondary/55 hover:text-red-300"
          >
            Huỷ
          </button>
        )}
        <button
          ...
          className="rounded-lg border border-cyan-300/30 bg-cyan-400/10 px-3 py-2 text-cyan-200 hover:bg-cyan-400/20 disabled:opacity-40"
        >
          <Send size={14} />
        </button>
      </div>
  ...
  return (
    <div className="rounded-lg border border-white/8 bg-white/[0.02] px-3 py-2 space-y-1">
      <div className="flex items-center justify-between gap-2">
        <span className="font-tech text-[10px] text-cyan-200">
          {comment.display_name ?? 'Ẩn danh'}
        </span>
        <span className="font-tech text-[9px] text-secondary/40">
      ...
      <div className="flex gap-3">
        {onReply && (
          <button
            onClick={onReply}
            className="inline-flex items-center gap-1 font-tech text-[9px] text-secondary/50 hover:text-cyan-200"
          >
        ...
        {comment.user_id === userId && (
          <button
            onClick={onDelete}
            className="inline-flex items-center gap-1 font-tech text-[9px] text-red-400/60 hover:text-red-300"
          >
            <Trash2 size={10} /> Xoá
          </button>
        )}
