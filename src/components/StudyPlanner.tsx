...
export default function StudyPlanner({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    getCurrentGoal(userId).then((g) => {
      setGoal(g);
      setLoading(false);
      if (g)
        setTargets({
          lessons_target: g.lessons_target,
          flashcards_target: g.flashcards_target,
          quizzes_target: g.quizzes_target,
        });
    });
  }, [userId]);
    ...
    return (
      <div className="glass-card rounded-2xl p-5 space-y-3">
        ...
        <p className="text-sm text-secondary/70">
          Đặt mục tiêu học tập cho tuần này để theo dõi tiến độ.
        </p>
        <button
    ...
    return (
      <div className="glass-card rounded-2xl p-5 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          <GoalInput
            label="Bài học"
            value={targets.lessons_target}
            onChange={(v) => setTargets((t) => ({ ...t, lessons_target: v }))}
          />
          <GoalInput
            label="Flashcards"
            value={targets.flashcards_target}
            onChange={(v) => setTargets((t) => ({ ...t, flashcards_target: v }))}
          />
          <GoalInput
            label="Quiz"
            value={targets.quizzes_target}
            onChange={(v) => setTargets((t) => ({ ...t, quizzes_target: v }))}
          />
        </div>
        <button
          onClick={save}
        ...
        {progress.map((p) => {
          ...
          return (
            <div key={p.label} className="text-center space-y-1">
              ...
              <p className="font-tech text-[9px] uppercase tracking-[0.14em] text-secondary/55">
                {p.label}
              </p>
            </div>
          );
...
function GoalInput({
  ...
  return (
    <div className="space-y-1">
      <label className="font-tech text-[9px] uppercase tracking-[0.14em] text-secondary/55">
        {label}
      </label>
      <input
        type="number"
        ...
        className="w-full rounded-lg border border-white/10 bg-white/[0.03] px-2 py-1.5 text-sm text-on-surface text-center focus:border-cyan-300/40 focus:outline-none"
      />
    </div>
  );
}
