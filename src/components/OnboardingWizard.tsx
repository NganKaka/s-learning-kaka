...
export default function OnboardingWizard({
  ...
  const finish = async () => {
    await upsertGoal(userId, {
      lessons_target: goals.lessons,
      flashcards_target: goals.flashcards,
      quizzes_target: goals.quizzes,
    });
        ...
        >
          {step === 'welcome' && (
            <>
              ...
              <h2 className="font-headline text-2xl font-extrabold text-on-surface">
                Chào mừng{displayName ? `, ${displayName}` : ''}!
              </h2>
              <p className="text-secondary/70">
                Hãy thiết lập mục tiêu học tập để bắt đầu hành trình.
              </p>
              <button
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1">
                  <label className="font-tech text-[9px] uppercase text-secondary/55">
                    Bài học
                  </label>
                  <input
                    type="number"
                    ...
                    value={goals.lessons}
                    onChange={(e) =>
                      setGoals((g) => ({ ...g, lessons: Math.max(1, +e.target.value) }))
                    }
                    className="w-full rounded-lg border border-white/10 bg-white/[0.03] px-2 py-2 text-center text-sm text-on-surface focus:border-cyan-300/40 focus:outline-none"
                  />
                </div>
                <div className="space-y-1">
                  <label className="font-tech text-[9px] uppercase text-secondary/55">
                  ...
                  <input
                    type="number"
                    min={1}
                    value={goals.flashcards}
                    onChange={(e) =>
                      setGoals((g) => ({ ...g, flashcards: Math.max(1, +e.target.value) }))
                    }
                    className="w-full rounded-lg border border-white/10 bg-white/[0.03] px-2 py-2 text-center text-sm text-on-surface focus:border-cyan-300/40 focus:outline-none"
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    value={goals.quizzes}
                    onChange={(e) =>
                      setGoals((g) => ({ ...g, quizzes: Math.max(1, +e.target.value) }))
                    }
                    className="w-full rounded-lg border border-white/10 bg-white/[0.03] px-2 py-2 text-center text-sm text-on-surface focus:border-cyan-300/40 focus:outline-none"
                  />
                </div>
          ...
          {step === 'done' && (
            <>
              ...
              <button
                onClick={onComplete}
                className="inline-flex items-center gap-2 rounded-full border border-emerald-400/40 bg-emerald-500/15 px-6 py-3 text-sm font-tech uppercase tracking-[0.16em] text-emerald-200 hover:bg-emerald-500/25"
              >
                Vào học
              </button>
            </>
