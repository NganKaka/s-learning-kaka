    ...
    <div className="space-y-4">
          ...
          <div className="flex items-center gap-1">
            {[1, 2, 3, 4, 5].map((s) => (
              <Star
                ...
                size={12}
                className={
                  s <= Math.round(avg) ? 'text-amber-400 fill-amber-400' : 'text-secondary/30'
                }
              />
            ))}
            <span className="font-tech text-[10px] text-secondary/55 ml-1">
      ...
      {enrolled && !hasReviewed && userId && (
        <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 space-y-3">
          <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((s) => (
              <button key={s} onClick={() => setMyRating(s)} className="p-0.5">
                <Star
                  size={20}
                  className={s <= myRating ? 'text-amber-400 fill-amber-400' : 'text-secondary/30'}
                />
              </button>
            ))}
          </div>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full border border-amber-400/40 bg-amber-500/15 px-4 py-2 text-xs font-tech uppercase tracking-[0.16em] text-amber-200 hover:bg-amber-500/25 disabled:opacity-50"
          >
            Gửi đánh giá
          </button>
        </div>
      ...
      <div className="space-y-3">
        {reviews.map((r) => (
          <div
            key={r.id}
            className="rounded-lg border border-white/8 bg-white/[0.02] px-4 py-3 space-y-1"
          >
            <div className="flex items-center gap-2">
              <span className="text-sm font-bold text-on-surface">
                {r.display_name ?? 'Học viên'}
              </span>
              <div className="flex">
                {[1, 2, 3, 4, 5].map((s) => (
                  <Star
                    ...
                    size={10}
                    className={
                      s <= r.rating ? 'text-amber-400 fill-amber-400' : 'text-secondary/30'
                    }
                  />
                ))}
              </div>
            ...
            {r.instructor_reply && (
              <p className="text-xs text-cyan-200/80 mt-1 pl-3 border-l-2 border-cyan-300/30">
                {r.instructor_reply}
              </p>
            )}
          </div>
