      ...
      <div className="grid sm:grid-cols-3 gap-3">
        {courses.map((c) => (
          <Link
            key={c.slug}
            to={`/courses/${c.slug}`}
            className="glass-card rounded-xl p-4 hover:border-cyan-300/30 transition-colors group"
          >
            <p className="text-sm font-bold text-on-surface group-hover:text-cyan-200 transition-colors">
              {c.title}
            </p>
