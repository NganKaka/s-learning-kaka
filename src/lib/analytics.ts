...
export async function computeEngagementScore(
  userId: string,
): Promise<{ composite: number; breakdown: Record<string, number> }> {
  ...
  // Flashcard reviews
  const { count: cardReviews } = await supabase
    .from('card_reviews')
    .select('*', { count: 'exact', head: true })
    .eq('user_id', userId)
  ...
  const completionRate =
    (totalEnrolled ?? 0) > 0
      ? Math.min(100, ((lessonsCompleted ?? 0) / ((totalEnrolled ?? 1) * 10)) * 100)
      : 0;
  const composite =
    loginFreq * 0.2 +
    completionRate * 0.3 +
    avgScore * 0.3 +
    Math.min(100, ((cardReviews ?? 0) / 50) * 100) * 0.2;
  ...
  await supabase.from('engagement_scores').upsert(
    {
      ...
      flashcard_reviews: cardReviews ?? 0,
  ...
  return {
    composite,
    breakdown: { loginFreq, completionRate, avgScore, cardReviews: cardReviews ?? 0 },
  };
}
...
export async function getFunnel(): Promise<{
  ...
  for (const p of heavyUsers ?? []) {
    userCounts.set(p.user_id as string, (userCounts.get(p.user_id as string) ?? 0) + 1);
  }
  const completed = [...userCounts.values()].filter((c) => c >= 10).length;

  return {
    visited: (signedUp ?? 0) * 3,
    signedUp: signedUp ?? 0,
    enrolled: enrolled ?? 0,
    started,
    completed,
...
export async function getCohortData(): Promise<
  Array<{ month: string; signups: number; completionRate: number }>
> {
