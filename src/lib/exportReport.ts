...
export async function exportStudentReport(courseId: string, instructorId: string): Promise<string> {
  ...
  const { data: attempts } = await supabase
    .from('quiz_attempts')
    .select('user_id, auto_score, final_score')
    .in('quiz_id', quizIds)
    .in('status', ['submitted', 'graded']);

  const progressByUser = new Map<string, number>();
  for (const p of progress ?? [])
    progressByUser.set(p.user_id as string, (progressByUser.get(p.user_id as string) ?? 0) + 1);
  ...
  const rows = [
    ['Tên', 'Bài hoàn thành', 'Tổng bài', '% Hoàn thành', 'Điểm TB quiz', 'Số lượt thi'].join(','),
  ];
  for (const uid of userIds) {
    ...
    const avg =
      scores.length > 0 ? (scores.reduce((s, v) => s + v, 0) / scores.length).toFixed(1) : '—';
