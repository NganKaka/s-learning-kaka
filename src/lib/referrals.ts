...
export async function applyReferral(
  ...
  await supabase
    .from('referrals')
    .insert({ referrer_id: referrer.id, referred_id: referredUserId, referral_code: referralCode });
  return { success: true };
}
...
export async function creditReferralReward(referrerId: string, amount: number): Promise<void> {
  ...
  if (data) {
    await supabase
      .from('profiles')
      .update({ wallet_balance_vnd: (data.wallet_balance_vnd as number) + amount })
      .eq('id', referrerId);
  }
}
...
export async function getMyReferrals(
  ...
  Array<{ id: string; referred_id: string; reward_credited: boolean; created_at: string }>
> {
  const { data } = await supabase
    .from('referrals')
    .select('*')
