...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    getMessages(user.id, selected).then((m) => {
      setMessages(m);
      markRead(user.id, selected);
    });
  }, [user, selected]);
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-bold text-on-surface truncate">
                      {c.displayName ?? 'Người dùng'}
                    </span>
                    {c.unread > 0 && (
                      <span className="shrink-0 w-5 h-5 rounded-full bg-primary/80 text-background text-[10px] font-bold flex items-center justify-center">
                        {c.unread}
                      </span>
                    )}
                  </div>
              ...
              <>
                <div className="flex-1 overflow-y-auto space-y-2 mb-3 max-h-[380px]">
                  {messages.map((m) => (
                    ...
                    >
                      <div
                        className={`max-w-[70%] rounded-xl px-3 py-2 text-sm ${m.sender_id === user.id ? 'bg-primary/15 text-cyan-100' : 'bg-white/[0.06] text-on-surface'}`}
                      >
                        {m.content}
                      </div>
                    </div>
