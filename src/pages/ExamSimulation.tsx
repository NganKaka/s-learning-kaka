...
export default function ExamSimulation() {
  ...
  if (questions.length === 0)
    return (
      <PageShell>
        <div className="glass-card rounded-2xl p-8 text-center text-secondary/65">
          Khoá học chưa có câu hỏi trắc nghiệm.
        </div>
      </PageShell>
    );
      ...
      <PageShell>
        <div className="max-w-2xl mx-auto glass-card rounded-2xl p-8 text-center space-y-4">
          ...
          <p className="text-secondary/70">
            {questions.length} câu · {questions.length} phút · Trắc nghiệm từ toàn bộ khoá học
          </p>
          <button
    ...
    <PageShell>
      <div className="max-w-3xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          ...
          {timeLeft !== null && (
            <span
              className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 font-tech text-[11px] tabular-nums ${timeLeft <= 60 ? 'border-red-400/50 text-red-200 animate-pulse' : 'border-cyan-300/40 text-cyan-200'}`}
            >
