        ...
        {stats.map((s) => {
          ...
          const qStats = s.questions
            .map((q) => {
              ...
              return {
                prompt: q.prompt_md,
                rate: s.attempts.length > 0 ? (correct / s.attempts.length) * 100 : 0,
              };
            })
                ...
                <div className="flex items-end gap-1 h-16">
                  {buckets.map((count, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      ...
                      />
                      <span className="font-tech text-[8px] text-secondary/45">
                        {i * 20}-{(i + 1) * 20}
                      </span>
                    </div>
                  ))}
                  ...
                  {qStats.slice(0, 3).map((qs, i) => (
                    <div key={i} className="flex items-center gap-3">
                      <span className="text-xs text-on-surface truncate flex-1">
                        {qs.prompt.slice(0, 60)}
                      </span>
                      <span
                        className={`font-tech text-[10px] tabular-nums ${qs.rate < 40 ? 'text-red-300' : 'text-secondary/60'}`}
                      >
